function im_aligned = AP_align_widefield_old(animal,days,im_unaligned)
...
%
% NOTE: This is from the first submission of the ctx-str paper,
% a more general version is now in AP_align_widefield
%
...
%% STORING HERE: Make master VFS (load in aligned, symmetrize)
...
if false
    ...
    n_loops = 10;
    ref_im = ref_retinotopy;
    ref_size = size(ref_im);
    
    ref_reg = nan(size(ref_im,1),size(ref_im,2),n_loops);
    
    for curr_loop = 1:n_loops
        ref_im_symm = (ref_im + fliplr(ref_im))./2;
        ...
        tformEstimate_affine = imregtform(ref_im,ref_im_symm,'rigid',optimizer,metric);
        curr_im_reg = imwarp(ref_im,tformEstimate_affine,'Outputview',imref2d(ref_size));
        tform_matrix = tformEstimate_affine.T;
        ref_im = curr_im_reg;
        
        ref_reg(:,:,curr_loop) = ref_im;
