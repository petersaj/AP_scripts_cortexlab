...
if task_dataset
    ...
    fluor_kernelroi_taskpred = permute(reshape( ...
        AP_svd_roi(U_master(:,:,1:n_vs), ...
        reshape(permute(fluor_taskpred_allcat,[3,2,1]),n_vs,[]),[],[],kernel_roi), ...
        size(kernel_roi,3),[],size(fluor_taskpred_allcat,1)),[3,2,1])./fluor_kernelroi_deconv_std;   
    fluor_kernelroi_taskpred_reduced = cell2mat(permute(arrayfun(@(x) ...
        ...
        1:size(fluor_taskpred_reduced_allcat,4),'uni',false),[1,3,4,2]))./fluor_kernelroi_deconv_std;
