...
% % (look in server 3) TEMPORARY: ZNAS BROKEN
...
%%%%%%%%%%%%%%%%%%%%%%%%% WHILE ZNAS BROKEN
% (look in temp clone for old znas data)
expInfo_path = ['\\znasclone.cortexlab.net\Subjects\' animal];
expInfo_dir = dir(expInfo_path);
day_paths = cellfun(@(x) ~isempty(regexp(x,'\d\d\d\d-\d\d-\d\d')),{expInfo_dir.name}) &...
    [expInfo_dir.isdir];
curr_days = {expInfo_dir(day_paths).name};
