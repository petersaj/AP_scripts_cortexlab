function experiments = AP_find_experiments_externalhd(animal,protocol,flexible_name)
...
for curr_day = 1:length(days)  
    ...
    if ~isempty(protocol)
        ...
        for curr_exp = 1:length(exp_nums)
            ...
            [block_filename, block_exists] = AP_cortexlab_filename_externalhd(animal,day,exp_nums(curr_exp),'block');
            [protocol_filename,protocol_exists] = AP_cortexlab_filename_externalhd(animal,day,exp_nums(curr_exp),'protocol');
    ...
    if any(use_exp)
        ...
        imaging_path = AP_cortexlab_filename_externalhd(animal,day,exp_nums(use_exp),'imaging');
        imaging_expts{curr_day} = exist([imaging_path filesep 'meanImage_blue.npy'],'file') > 0;
        [~,ephys_expts{curr_day}] = AP_cortexlab_filename_externalhd(animal,day,exp_nums(use_exp),'ephys_dir');
    end
