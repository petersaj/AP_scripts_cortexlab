...
% Cell types in dim 4 (wide, narrow)
...
%         vertcat(mua_depth_day_baseline{:}),'uni',false));
    ...
    if task_dataset && exist('mua_taskpred_all','var')
        ...
        mua_taskpred_reduced_norm_reorder = cellfun(@(x) ...
            nan(size(x,1),size(x,2),length(mua_areas),length(task_regressor_labels)), ...
            mua_taskpred_reduced_norm,'uni',false);
    end
