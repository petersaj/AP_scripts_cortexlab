...
if exist('mua_area_all','var')    
    ...
    % (baseline = mean quiescent pre-stim)
    norm_use_trials = cellfun(@(x) ~any(x(:,t_baseline),2), ...
        vertcat(wheel_all{:}),'uni',false);
    mua_area_day_baseline = cellfun(@(mua,use_trials) ...
        nanmean(reshape(mua(use_trials,t_baseline,:), ...
        1,[],size(mua,3),size(mua,4)),2), ...
        vertcat(mua_area_all{:}),norm_use_trials,'uni',false);
    
    % (soften norm: +10%ile rate across all area/recordings)
    mua_norm_soften = ...
        prctile(cat(3,mua_area_day_baseline{:}),10);

    % (get norm)
    mua_area_norm = cellfun(@(mua,mua_baseline) ...
        (mua-mua_baseline)./(mua_baseline + mua_norm_soften), ...
        vertcat(mua_area_all{:}), ...
        mua_area_day_baseline,'uni',false);
