...
while i < length(curr_wheel_mm)
    ...
    thresh = 1; % (hardcoded in protocol)
    next_thresh_cross = min([length(curr_wheel_mm_diff),(i-1) + ...
        find(cumsum(curr_wheel_mm_diff(i:end)) > thresh,1,'first')]);
...
reconstructed_stimOn_leeway = 0.01;
curr_reconstructed_stimOn = ...
    curr_trial_t(find(t_from_quiescence_reset_trial_block > curr_quiescence,1));
curr_reconstructed_stimOn_error = ...
    curr_reconstructed_stimOn - signals_events.stimOnTimes(curr_trial);
