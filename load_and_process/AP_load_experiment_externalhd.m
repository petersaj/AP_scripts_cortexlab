...
%
% EXTERNAL HD VERSION: look on external hd, after leaving lab so server not
% accessible
...
[timeline_filename,timeline_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'timeline');
if ~timeline_exists
...
[protocol_filename,protocol_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'protocol');
...
if protocol_exists
    ...
    hwinfo_filename = AP_cortexlab_filename_externalhd(animal,day,experiment,'hardware');
...
% Load the block
[block_filename, block_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'block');
...
if exist('Timeline','var') && load_parts.cam
    ...
    % EYECAM
    [eyecam_dir,eyecam_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'eyecam');
    ...
    if eyecam_exists
        ...
        % (from facemap)
        [eyecam_processed_filename,eyecam_processed_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'eyecam_processed');
        if eyecam_processed_exists
            eyecam = load(eyecam_processed_filename);
        end
        % (from deeplabcut)
        [eyecam_dlc_filename,eyecam_dlc_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'eyecam_dlc');
        ...
        % Get camera times
        eyecam_fn = AP_cortexlab_filename_externalhd(animal,day,experiment,'eyecam');
    ...
    % FACECAM
    [facecam_dir,facecam_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'facecam');
    
    if facecam_exists
        if verbose; disp('Loading facecam...'); end
        
        % Get camera times
        facecam_fn = AP_cortexlab_filename_externalhd(animal,day,experiment,'facecam');
        ...
        [facecam_processed_filename,facecam_processed_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'facecam_processed');
        if facecam_processed_exists
            facecam = load(facecam_processed_filename);
        end
        
        % (output from AP_mouse_movie_movement)
        [facecam_movement_filename,facecam_movement_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'facecam_movement');
...
[data_path,data_path_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'imaging');
...
if ~exist('kilosort_version','var') || kilosort_version == 2
    [ephys_path,ephys_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'ephys',site,recording);
elseif exist('kilosort_version','var') && kilosort_version == 1
    [ephys_path,ephys_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'ephys_ks1',site,recording);
end
...
if ephys_exists && load_parts.ephys && exist('lfp_channel','var')
    ...
    [data_path,data_path_exists] = AP_cortexlab_filename_externalhd(animal,day,experiment,'ephys_dir',site);
...
elseif ephys_exists && load_parts.ephys && exist('ephys_align','var') && ...
    ...
    [probe_ccf_fn,probe_ccf_fn_exists] = AP_cortexlab_filename_externalhd(animal,[],[],'probe_ccf');
