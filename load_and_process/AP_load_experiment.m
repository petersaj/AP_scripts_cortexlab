...
if ephys_exists && load_parts.ephys
    ...
    mua_median_factor = 3;
    median_corr = medfilt1(nanmedian(mua_corr_end,2),mua_median_factor);
    [x,max_corr_drop] = min(diff(median_corr));
    str_end = depth_group_centers(end-groups_back+max_corr_drop-mua_median_factor-1);
    ...
    if any(curr_animal_idx)
        ...
        if any(curr_day_idx)
            ...
            % (get striatum depth boundaries from the end of the striatum relative to the across-animal size)
            str_depth_edges = sort(str_depth(2) - ephys_align(curr_animal_idx).str_depth_edges);
