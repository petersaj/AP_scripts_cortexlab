...
% type - grid, ccf, ccf_aligned, retinotopy (master only at the moment)
...
switch type   
    ...
    case 'ccf'        
        ...
        for curr_area_idx =1:length(top_down_cortical_area_boundaries)
            h = cellfun(@(outline) plot(bregma_x + ((outline(:,2)-bregma(3))*10)/um2pixel, ...
                bregma_y + ((outline(:,1)-bregma(1))*10)/um2pixel,color),cortical_area_boundaries{curr_area_idx},'uni',false);
        end
