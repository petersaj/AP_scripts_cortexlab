...
%% Choiceworld trial activity (TESTING SQRT STR SPIKES)
...
%             true,regression_params.use_constant);       
        ...
        %%% TESTING: Regress cortex to sqrt(striatum)
        binned_spikes_std_sqrt = sqrt(binned_spikes_std);
        
        [ctx_str_k,ctxpred_spikes_std_sqrt,explained_var] = ...
            AP_regresskernel(fVdf_deconv_resample(regression_params.use_svs,:), ...
            binned_spikes_std_sqrt,kernel_frames,lambda, ...
            regression_params.zs,regression_params.cvfold, ...
            true,regression_params.use_constant);  
        
        ctxpred_spikes_std = ctxpred_spikes_std_sqrt.^2;
        ctx_str_k{end} = ctx_str_k{end}.^2;
...
save_path = 'C:\Users\Andrew\OneDrive for Business\Documents\CarandiniHarrisLab\analysis\wf_ephys_choiceworld\paper\data';
save_fn = ['trial_activity_choiceworld_sqrtstr'];
save([save_path filesep save_fn],'-v7.3');
...
%% Fig 4g: Cortex-predicted striatum error (RANK)
...
for curr_area_idx = 1:length(plot_areas)
    ...
    for curr_timeavg = 1:length(timeavg_labels)
        ...
        curr_act_pred_condition_diff_shuff(:,:,curr_timeavg) = ...
            ...
            curr_act_avg_rank,curr_act_pred_avg_rank,trial_conditions_exp_shuff,use_trials), ...
            1:n_shuff,'uni',false));
