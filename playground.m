...
for curr_animal = 1:length(animals)
    ...
    for curr_day = 1:length(experiments)
        ...
        % Get reaction time
        [move_trial,move_idx] = max(abs(event_aligned_wheel(use_trials,:)) > 2,[],2);
        move_t = t(move_idx)';
        ...
        % Reward regressors
        curr_reward_grid = event_aligned_reward(use_trials,:,:);
        reward_regressor = zeros(sum(use_trials),length(t));
        for curr_trial = 1:sum(use_trials)
            curr_reward = find(curr_reward_grid(curr_trial,:));
            for i = curr_reward
                curr_reward_t = t(i);
        ...
        regressors = {stim_regressors;move_onset_regressors;go_cue_regressors;reward_regressor};
