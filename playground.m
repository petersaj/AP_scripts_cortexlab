...
% (stim)
a = nanmean(curr_act_pred_allcat(trial_stim_allcat == 1,t > 0 & t < 0.15,plot_areas),2);
...
%% Testing metric histogram

msn = logical(alldata.msn);
fsi = logical(alldata.fsi);
tan = logical(alldata.tan);
uin = logical(alldata.uin);

pss = log(alldata.pss_allcat2);
spikewidth = alldata.templateDuration_all;
...
figure; hold on;
for curr_celltype = 1:size(pDcounts,3)
    curr_pd = pDcounts(:,:,curr_celltype);
    i = find(curr_pd);
    [a,b] = find(curr_pd);
    scatter(a,b,curr_pd(i)*5000,'filled');
end


celltype_col = ...
    [1.0000,0.4102,0.7031; ...
    1.0000,0.5469,0; ...
