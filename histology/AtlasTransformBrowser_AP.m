function AtlasTransformBrowser_AP(templateVolume, annotationVolume, structureTree, save_location, save_suffix)
% ------------------------------------------------
...
% Initialize 'ud' for guidata
ud = struct;

% Load in slice images
slice_im_dir = dir([save_location filesep '*.tif']);
...
ud.gui_fig = figure;
ud.slice_ax = subplot(1,3,1); axis image off;
ud.atlas_ax = subplot(1,3,2); axis image off;
ud.slice_atlas_overlay_ax = subplot(1,3,3); axis image off;

axes(ud.slice_ax);
ud_slice_obj = image(slice_im{1},'Parent',ud.slice_ax); 
...
set(ud.im, 'ButtonDownFcn', @(f,k)atlasClickCallback(f, k, ud.slice_ax, save_location));
...
set(ud.gui_fig, 'UserData', ud);
set(ud.gui_fig, 'KeyPressFcn', @(f,k)hotkeyFcn(f, ud.slice_ax, k, allData, save_location, save_suffix));
set(ud.gui_fig, 'WindowScrollWheelFcn', @(src,evt)updateSlice(f, evt, allData, ud.slice_ax, save_location))
set(ud.gui_fig, 'WindowButtonMotionFcn',@(f,k)fh_wbmfcn(f, allData, ud.slice_ax, save_location)); % Set the motion detector.
...
msgbox( ...
    {'\fontsize{12}' ...
    '\bf Controls: \rm' ...
    'scroll: move between slices' ...
    'g: add/remove gridlines' ...
    'o: add/remove overlay of current region extent' ...
    'a: toggle viewing boundaries' ...
    'v: toggle color atlas mode' ...
    'p: enable/disable mode where clicks are logged for probe or switch probes' ...
    'n: trace a new probe' ...
    'l: load saved probe points' ...
    's: save current probe' ...
    'w: enable/disable probe viewer mode for current probe' ...
    'd: delete most recent probe point' ...
    'up: scroll through A/P angles' ...
    'right: scroll through M/L angles' ...
    'down: scroll through slices'}, ...
