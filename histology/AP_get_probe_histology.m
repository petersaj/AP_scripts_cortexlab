...
switch eventdata.Key
    ...
    case 'escape'
        ...
        if strcmp(user_confirm,'Yes')
            ...
            probe_ccf = struct('points',cell(gui_data.n_probes,1), ...
                'areas',cell(gui_data.n_probes,1));
            ...
            for curr_probe = 1:gui_data.n_probes
                ...
                trajectory_coords_outofbounds = ...
                    any([trajectory_xcoords;trajectory_ycoords;trajectory_zcoords] < 1,1) | ...
                    any([trajectory_xcoords;trajectory_ycoords;trajectory_zcoords] > size(gui_data.av)',1);
                                
                trajectory_coords_idx = sub2ind(size(gui_data.av), ...
                    trajectory_xcoords(~trajectory_coords_outofbounds), ...
                    trajectory_ycoords(~trajectory_coords_outofbounds), ...
                    trajectory_zcoords(~trajectory_coords_outofbounds));
                
                trajectory_areas = gui_data.av(trajectory_coords_idx)';
                
                % Get rid of any non-brain areas
                trajectory_areas(isnan(trajectory_areas) | trajectory_areas <= 1) = [];
        
                probe_ccf(curr_probe).areas = trajectory_areas;
...
for curr_probe = 1:length(probe_ccf)
    ...
    trajectory_area_labels = gui_data.st.safe_name(probe_ccf(curr_probe).areas(round(trajectory_area_centers)));
      
    image(probe_ccf(curr_probe).areas);
