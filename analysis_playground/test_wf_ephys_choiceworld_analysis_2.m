...
% for mua: taskpred is off by condition and ctxpred is matched??
% that's the opposite of what I thought it should be
    ...
    trial_conditions = trial_condition_groups{curr_area};
        ...
        for curr_timeavg = 1:length(timeavg_labels)
            ...
            act_totalmean = cell2mat(arrayfun(@(condition) ...
                ...
                act(use_trials(:,area) & trial_cond(:,condition),area), ...
                [1,1],@nanmean,cast(NaN,class(act))), 1:size(act,2),'uni',false)), ...
                curr_act_avg,total_bins,trial_conditions_exp,nonan_trials,'uni',false),[2,3,1])), ...
                permute(1:size(trial_conditions,2),[1,3,4,2]),'uni',false));
            ...
            act_pred_totalmean = cell2mat(arrayfun(@(condition) ...
                ...
                act(use_trials(:,area) & trial_cond(:,condition),area), ...
                [1,1],@nanmean,cast(NaN,class(act))), 1:size(act,2),'uni',false)), ...
                curr_act_pred_avg,total_bins,trial_conditions_exp,nonan_trials,'uni',false),[2,3,1])), ...
                permute(1:size(trial_conditions,2),[1,3,4,2]),'uni',false));
            
            % Get the explained variance by condition
            expl_var = cell2mat(permute(cellfun(@(act,pred,conditions) cell2mat(arrayfun(@(condition) ...
                1-(nansum((act(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)) & conditions(:,condition),:) - ...
                pred(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)) & conditions(:,condition),:)).^2,1)./ ...
                nansum((act(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)) & conditions(:,condition),:) - ...
                nanmean(act(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)) & conditions(:,condition),:))).^2,1)), ...
                [1:size(trial_conditions,2)]','uni',false)), ...
                curr_act_avg,curr_act_pred_avg,trial_conditions_exp,'uni',false),[2,3,1]));
            
            % Plot binned predicted v measured
            figure(measured_v_pred_fig);
            subplot(length(plot_areas),length(timeavg_labels), ...
                sub2ind(fliplr([length(plot_areas),length(timeavg_labels)]),curr_timeavg,curr_area));
            hold on;
            
            errorbar( ...
                squeeze(nanmean(act_binmean(:,plot_area,:,:),3)), ...
                squeeze(nanmean(act_pred_binmean(:,plot_area,:,:),3)), ...
                squeeze(AP_sem(act_pred_binmean(:,plot_area,:,:),3)), ...
                'linewidth',2,'CapSize',0);
            errorbar( ...
                squeeze(nanmean(act_totalmean(:,plot_area,:,:),3)), ...
                squeeze(nanmean(act_pred_totalmean(:,plot_area,:,:),3)), ...
                squeeze(AP_sem(act_pred_totalmean(:,plot_area,:,:),3)), ...
                squeeze(AP_sem(act_pred_totalmean(:,plot_area,:,:),3)), ...
...
% Link axes of all plots
linkaxes(get(measured_v_pred_fig,'Children'));



%% (working on: R^2 from above)

expl_var_all = cell2mat(cellfun(@(act,pred) ...
    1-(nansum((act(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)),:) - ...
            pred(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)),:)).^2,1))./ ...
            nansum((act(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)),:) - ...
    nanmean(act(~isnan(act(:,plot_area)) & ~isnan(pred(:,plot_area)),:))).^2,1), ...
    curr_act_avg,curr_act_pred_avg,'uni',false));
...
figure; 
errorbar(nanmean(expl_var,3)',AP_sem(expl_var,3)');



plot_c = 1;
a = cell2mat(curr_act_avg); a = a(trial_conditions(:,plot_c),plot_area);
b = cell2mat(curr_act_pred_avg); b = b(trial_conditions(:,plot_c),plot_area);

sse_residual = nansum((a-b).^2);
sse_total = nansum((a-nanmean(a)).^2);

expl_var = 1 - (sse_residual./sse_total);
disp(expl_var)
