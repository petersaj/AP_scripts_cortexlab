%% Retinotopy via sweep fourier (set to use for hemodynamic - mpep)
...
vfs = nanmean(vfs_boot,3);
amp = nanmean(amp_boot,3);

figure('Name',animal);
ax1 = axes;
subplot(1,2,1,ax1);
h1 = imagesc(vfs);
set(h1,'AlphaData',mat2gray(amp,[0,prctile(amp(:),90)]));
caxis([-1,1]);
axes(ax1); axis image off;
colormap(colormap_BlueWhiteRed)
