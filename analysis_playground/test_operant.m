...
AP_imscroll([stim_avg_naive,stim_avg_unlearned,stim_avg_learned]);
caxis([-max(abs(caxis)),max(abs(caxis))]);
colormap(brewermap([],'PrGn'));
axis image;
...
AP_imscroll(reshape(permute(curr_px,[1,2,4,3,5]), ...
...
AP_imscroll(stim_px_avg_learnedday,t);
caxis([-max(abs(caxis)),max(abs(caxis))]);
colormap(brewermap([],'PrGn'));
axis image;
AP_reference_outline('ccf_aligned',[0.5,0.5,0.5]);
...
use_trials = trial_learned_day == 0
...
%% Average ROI and wheel by reaction time and learned day
...
use_trials = ~any(isnan(reshape(accum_stage_idx,sum(trials_recording),[])),2);
...
%% Average ROI and wheel by binned wheel over learned days

plot_roi = 6;
plot_learned_days = [-2,-1,0];

use_t = t >= 0.1 & t <= 0.2;
wheel_leftward_tmax = -min(wheel_allcat(:,use_t),[],2);
wheel_bin_edges = [0,0,prctile(wheel_leftward_tmax(wheel_leftward_tmax > 0),20:20:80)];
n_wheel_bins = length(wheel_bin_edges)-1;
wheel_bins = discretize(wheel_leftward_tmax,wheel_bin_edges);

bin_col = copper(n_wheel_bins);
day_col = circshift(copper(length(plot_learned_days)),[0,-1]);

figure; 
h = tiledlayout(length(plot_learned_days),2);
h_avg = figure; hold on; set(gca,'ColorOrder',day_col);
for curr_learned_day = plot_learned_days

    use_trials = ~isnan(wheel_bins) & trial_learned_day == curr_learned_day;

    roi_hemiflip = circshift(1:n_rois,n_rois/2);
    curr_act = fluor_roi_deconv(use_trials,:,plot_roi) - ...
        fluor_roi_deconv(use_trials,:,roi_hemiflip(plot_roi));
    curr_wheel = -wheel_allcat(use_trials,:);

    % Get activity and wheel by wheel bin
    [wheel_bin_idx,t_idx] = ndgrid(wheel_bins(use_trials,:),1:length(t));

    curr_act_wheelbin_avg = accumarray( ...
        [wheel_bin_idx(:),t_idx(:)],curr_act(:),[n_wheel_bins,length(t)], ...
        @nanmean,nan(1,class(curr_act)));

    curr_wheel_wheelbin_avg = accumarray( ...
        [wheel_bin_idx(:),t_idx(:)],curr_wheel(:),[n_wheel_bins,length(t)], ...
        @nanmean,nan(1,class(curr_wheel)));

    % Plot activity and wheel binned timecourses
    nexttile(h); hold on; set(gca,'ColorOrder',bin_col);
    plot(t,curr_act_wheelbin_avg','linewidth',2)
    line([0,0],ylim,'color','k','linestyle','--');
    ylabel(wf_roi(plot_roi).area);

    nexttile(h); hold on; set(gca,'ColorOrder',bin_col);
    plot(t,curr_wheel_wheelbin_avg','linewidth',2)
    line([0,0],ylim,'color','k','linestyle','--');
    ylabel('Wheel');

    % Plot activity by wheel
    figure(h_avg); hold on;
    plot(max(curr_wheel_wheelbin_avg(:,use_t),[],1), ...
        max(curr_act_wheelbin_avg(:,use_t),[],1),'linewidth',2);
    xlabel('Wheel');
    ylabel(wf_roi(plot_roi).area);

end
% (link modality axes)
ax = reshape(flipud(allchild(h)),[],length(plot_learned_days))';
for curr_roi = 1:2
...
% Get kernel for movements during ITI
move_iti_k = cellfun(@(x) cellfun(@(x) permute(x{2}(2,:,:),[3,2,1]), ...
    x,'uni',false),fluor_taskpred_k_all,'uni',false);

move_iti_k_avg = cellfun(@(x) nanmean(cat(3,x{:}),3),move_iti_k,'uni',false);

move_iti_px_grandavg = AP_svdFrameReconstruct(U_master(:,:,1:n_vs), ...
    nanmean(cat(3,move_iti_k_avg{:}),3));

AP_imscroll(move_iti_px_grandavg);
colormap(AP_colormap('KWG',[],1.5));
caxis(max(abs(caxis))*[-1,1]);
axis image off;

% Get move iti kernels in ROIs
...
px_ratio = cell2mat(permute(cellfun(@(x) ...
    nanmedian(x./(AP_reflect_widefield(x)),3),px,'uni',false),[2,3,1]));

figure;
imagesc(nanmean(px_ratio(:,1:round(size(U_master,2)/2),:),3))
caxis([0,2]);
colormap(AP_colormap('BWR',[],1));
c = colorbar;
ylabel(c,'L/R ratio during movement')
...
AP_imscroll(fluor_muscimol_avg_px,t)
caxis([-max(abs(caxis)),max(abs(caxis))]);
colormap(brewermap([],'PrGn'));
axis image;
AP_reference_outline('ccf_aligned',[0.5,0.5,0.5]);
