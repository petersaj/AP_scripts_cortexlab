...
% Set cells to plot
plot_cells = find(stim_cells & celltype_allcat == 1 & domain_aligned_allcat == 1);
...
use_trials = cellfun(@(stim,rxn,outcome) ...
    stim > 0 & rxn < 0.5 & outcome == 1, ...
    trial_stim_allcat_exp,move_t_exp,trial_outcome_allcat_exp,'uni',false);
[~,max_aligned_idx_exp] = cellfun(@(use_trials,stim_aligned,move_aligned,outcome_aligned) ...
    max([ ...
    max(AP_deconv_wf(nanmean(stim_aligned(use_trials,:,:),1),true),[],2), ...
    max(AP_deconv_wf(nanmean(move_aligned(use_trials,:,:),1),true),[],2), ...
    max(AP_deconv_wf(nanmean(outcome_aligned(use_trials,:,:),1),true),[],2)],[],2), ...
    use_trials,mua_allcat_stimalign_exp, ...
    mua_allcat_movealign_exp,mua_allcat_outcomealign_exp,'uni',false);
max_aligned_idx = cell2mat(cellfun(@(x) reshape(x,[],1),max_aligned_idx_exp,'uni',false));
