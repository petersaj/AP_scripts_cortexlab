...
%% Linear regression on day-concatenated activity
...
fluor_activity_model_params = nan(11,length(t),n_rois,2,length(D_all));
mua_activity_model_params = nan(11,length(t),n_depths,2,length(D_all));
mua_predicted_activity_model_params = nan(11,length(t),n_depths,2,length(D_all));
...
%     stim_regressors = D.stimulus.^contrast_exp;   
    ...
    % (to use equal and opposite L/R regressor)
    choice_regressors = 2*(D.response-1.5);
%     % (to use separate L/R regressors)
...
%     choice_regressors(D.response == 2,2) = 1;
    ...
    n_regressors = size(cell2mat(regressors'),1);
