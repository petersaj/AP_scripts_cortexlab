...
samplerate = framerate*sample_rate_factor;
...
depth_edges = [500,1700];
sample_rate_factor = 2;

% % SIGNALS - CHOICEWORLD
% use_trials = signals_events.missValues == 1;
...
error_t = [0,0.3];
error_t_use = t_surround >= error_t(1) & t_surround <= error_t(2);
pred_error = nanmean(spikes_real_aligned(:,error_t_use) - spikes_pred_aligned(:,error_t_use),2);
[pred_error_mean,pred_error_sem] = grpstats(pred_error,stim_conditions,{'mean','sem'});
figure;errorbar(unique(stim_conditions),pred_error_mean,pred_error_sem,'k','linewidth',2)
...
align_times = reshape(stimOn_times(use_trials(1:length(stimOn_times))),[],1);
stim_conditions = signals_events.trialContrastValues(use_trials).* ...
    signals_events.trialSideValues(use_trials);
...
if exist('signals_events','var')
    ...
    line(spike_lim,spike_lim*fit_hit(2)+ ...
        fit_hit(1)','color',[0,0.8,0],'linewidth',2)
    line(spike_lim,spike_lim*fit_miss(2)+ ...
        fit_miss(1)','color',[0.8,0,0],'linewidth',2)
end
