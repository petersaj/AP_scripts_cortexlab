...
ctx_text_center = ctx_circle_radius_text*...
    [sind(ctx_circle_degs(1:n_rois))',cosd(ctx_circle_degs(1:n_rois))'];
scatter(ctx_plot_center(:,1),ctx_plot_center(:,2),200,copper(n_rois),'filled','MarkerEdgeColor','k');
...
% Fluor-MUA
subplot(1,3,3); hold on; axis image off;
set(gca,'YDir','reverse');
AP_reference_outline('ccf_aligned',[0.7,0.7,0.7]);AP_reference_outline('retinotopy',[0.7,0.7,1]);

wf_roi_center = nan(length(wf_roi),2);
for curr_roi = 1:length(wf_roi)
...
str_plot_center = [linspace(300,300,n_depths)',linspace(100,350,n_depths)'];
...
for curr_roi = 1:n_rois
    for curr_depth = 1:n_depths
        ...
        line([wf_roi_center(curr_roi,1),str_plot_center(curr_depth,1)], ...
            [wf_roi_center(curr_roi,2),str_plot_center(curr_depth,2)], ...
            'linewidth',curr_weight, ...
            'color',[curr_sign == 1,0,curr_sign == -1]);
