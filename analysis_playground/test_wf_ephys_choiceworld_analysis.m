...
% Spikes in striatum
use_spikes_idx = spikeDepths > 500 & spikeDepths < 1000;
...
% Load data
data_path = 'C:\Users\Andrew\OneDrive for Business\Documents\CarandiniHarrisLab\analysis\wf_ephys_choiceworld\choiceworld';
data_fn = ['all_trial_activity_Udf_kernel-str_4_depths_long.mat'];
% data_fn = 'all_trial_activity_df_msn_earlymove.mat';
...
% Add a third alignment: max velocity in chosen direction
t_leeway = 0.5;
...
fluor_allcat(:,:,:,3) = fluor_allcat(:,:,:,1);
for curr_trial = 1:size(fluor_allcat,1)
    fluor_allcat(curr_trial,:,:,3) = circshift(fluor_allcat(curr_trial,:,:,1),[0,-max_vel_idx(curr_trial)+leeway_samples]);
end

mua_allcat(:,:,:,3) = mua_allcat(:,:,:,1);
for curr_trial = 1:size(mua_allcat,1)
    mua_allcat(curr_trial,:,:,3) = circshift(mua_allcat(curr_trial,:,:,1),[0,-max_vel_idx(curr_trial)+leeway_samples]);
end

wheel_velocity_allcat(:,:,3) = wheel_velocity_allcat(:,:,1);
for curr_trial = 1:size(wheel_velocity_allcat,1)
    wheel_velocity_allcat(curr_trial,:,3) = circshift(wheel_velocity_allcat(curr_trial,:,1),[0,-max_vel_idx(curr_trial)+leeway_samples]);
end
...
%         curr_px(curr_px < 0) = 0;
        ...
        % Smoothed Fluorescence derivative
        smooth_factor = 3;
        curr_data = squeeze(nanmean(convn(fluor_allcat(curr_trials,:,:,plot_align), ...
            ones(1,smooth_factor)/smooth_factor,'same'),1))';
        curr_px = diff(svdFrameReconstruct(U_master(:,:,1:n_rois),curr_data),[],3);
        curr_px(curr_px < 0) = 0;
...
for curr_rxn = 1:length(rxn_time_bins)
    for curr_trial_type = 1:size(trial_types,2)
        ...
        curr_data = mua_allcat(curr_trials,:,:,plot_align);
...
for curr_rxn = 1:length(rxn_time_bins)
    for curr_trial_type = 1:size(trial_types,2)
        ...
        curr_data = wheel_velocity_allcat(curr_trials,:,plot_align);
...
act_diff = mat2gray([zeros(size(fluor_roi,1),1),fluor_roi],double([0,prctile(reshape(fluor_roi,[],1),95)])) - ...
    mat2gray(mua_allcat(:,:,2,2),[0,prctile(reshape(mua_allcat(:,:,3,2),[],1),95)]);
...
%%% 3 areas together
activity_cat = cat(3,fluor_allcat,mua_allcat);
area_labels = [{wf_roi.area},arrayfun(@(x) ...
    ['Str ' num2str(x)],1:4,'uni',false)];

activity_zero_L = {activity_cat( ... 
    trial_contrast_allcat == 0 & ...
    trial_choice_allcat == -1,:,:,:)};

activity_zero_R = {activity_cat( ... 
    trial_contrast_allcat == 0 & ...
    trial_choice_allcat == 1,:,:,:)};

activity_split_hit = arrayfun(@(x) activity_cat( ... 
    trial_side_allcat == 1 & ...
    trial_contrast_allcat == contrasts(x) & ...
    trial_choice_allcat == -1,:,:,:), ...
    2:length(contrasts),'uni',false);

activity_split_miss = arrayfun(@(x) activity_cat( ... 
    trial_side_allcat == -1 & ...
    trial_contrast_allcat == contrasts(x) & ...
    trial_choice_allcat == 1,:,:,:), ...
    2:length(contrasts),'uni',false);

activity_split_mean = cell2mat(cellfun(@(x) nanmean(x,1), ...
...
for plot_align = 1:2
    ...
    set(gca,'ColorOrder',[0.6,0.2,0.2;0.2,0.2,0.6; ...
        col(end-4:end,:);col(5:-1:1,:)]);
    arrayfun(@(x) ...
        plot3(activity_split_mean(x,:,plot_areas(1),plot_align), ...
        activity_split_mean(x,:,plot_areas(2),plot_align), ...
        activity_split_mean(x,:,plot_areas(3),plot_align),'linewidth',2),1:size(activity_split_mean,1));
    for i = 1:size(activity_split_mean,1)
        plot_t = find(t > 0,1);
        plot3(activity_split_mean(i,plot_t,plot_areas(1),plot_align), ...
            activity_split_mean(i,plot_t,plot_areas(2),plot_align), ...
            activity_split_mean(i,plot_t,plot_areas(3),plot_align),'.k','MarkerSize',20)
    end
...
legend([{'0L'},{'0R'}, ...
    cellfun(@(x) [num2str(x) 'L'],num2cell(contrasts(2:end)),'uni',false), ...
    cellfun(@(x) [num2str(x) 'R'],num2cell(contrasts(2:end)),'uni',false)]);


%%% Widefield activity in PC space
[coeff,score,latent] = pca(reshape(permute(fluor_unilateral_allcat,[2,1,4,3]),[],8));
score_r = permute(reshape(score,size(permute(fluor_unilateral_allcat,[2,1,4,3]))),[2,1,4,3]);
...
plot_align = 1;
col = copper(n_rxn_times);
figure; hold on;
set(gca,'ColorOrder',col);
arrayfun(@(x) ...
    plot3(activity_split_mean(x,:,1,plot_align), ...
    activity_split_mean(x,:,2,plot_align), ...
    activity_split_mean(x,:,3,plot_align),'linewidth',2),1:n_rxn_times);
view([45,45,45]);axis normal vis3d;

%%% by contrast
activity_zero_L = {score_r( ... 
    trial_contrast_allcat == 0 & ...
    trial_choice_allcat == -1,:,:,:)};

activity_zero_R = {score_r( ... 
    trial_contrast_allcat == 0 & ...
    trial_choice_allcat == 1,:,:,:)};

activity_split_hit = arrayfun(@(x) score_r( ... 
    trial_side_allcat == 1 & ...
    trial_contrast_allcat == contrasts(x) & ...
    trial_choice_allcat == -1,:,:,:), ...
    2:length(contrasts),'uni',false);

activity_split_miss = arrayfun(@(x) score_r( ... 
    trial_side_allcat == -1 & ...
    trial_contrast_allcat == contrasts(x) & ...
    trial_choice_allcat == 1,:,:,:), ...
    2:length(contrasts),'uni',false);
...
for plot_align = 1:2
    ...
    arrayfun(@(x) ...
        plot3(activity_split_mean(x,:,1,plot_align), ...
        activity_split_mean(x,:,2,plot_align), ...
        activity_split_mean(x,:,3,plot_align),'linewidth',2),1:size(activity_split_mean,1));
    for i = 1:size(activity_split_mean,1)
        plot_t = find(t > 0,1);
        plot3(activity_split_mean(i,plot_t,1,plot_align), ...
            activity_split_mean(i,plot_t,2,plot_align), ...
            activity_split_mean(i,plot_t,3,plot_align),'.k','MarkerSize',20)
    end
    view([45,45,45]);axis normal vis3d;
...
for curr_animal = 1:length(D_all)
    ...
    switch plot_act
        ...
        case 'wheel'
            use_data =  permute(wheel_cat_norm,[1,2,4,3]);
            area_labels = {'Wheel'};
    ...
    % Plot average activity across time for reference (move L)
    subplot(3,length(D_all),curr_animal); hold on;
    plot_trials = trial_choice == -1;
    condition_act = grpstats(use_data(plot_trials,:,plot_area,plot_align), ...
        trial_side(plot_trials).*trial_contrast(plot_trials));
    ...
    axis tight;
    ylabel(area_labels{plot_area});
    title('Average activity (move L)');
    
    % Plot average activity across time for reference (move R)
    subplot(3,length(D_all),length(D_all) + curr_animal); hold on;
    plot_trials = trial_choice == 1;
    condition_act = grpstats(use_data(plot_trials,:,plot_area,plot_align), ...
        trial_side(plot_trials).*trial_contrast(plot_trials));
    set(gca,'ColorOrder',colormap_BlueWhiteRed(length(contrasts)-1));
    ...
    axis tight;
    ylabel(area_labels{plot_area});
    title('Average activity (move R)');
    
    % Plot distribution plot
    subplot(3,length(D_all),length(D_all)*2 + curr_animal); hold on;
...
trial_act_timeavg_split_all_mad = cellfun(@(x) mad(x,1),trial_act_timeavg_split_all);
...
regressors = {stim_regressors;move_onset_regressors;reward_allcat_regressor};
...
% Do regression on fluor (not combined unilateral at the moment)
t_shifts = {[-0.1,0.5];[-0.2,0.2];[-0.2,0.5]};
...
fluor_allcat_predicted = nan(size(fluor_allcat_downsamp));

fluor_kernel = cell(length(regressors),n_rois);
fluor_expl_var = cell(length(regressors),n_rois);
for curr_roi = 1:n_rois
    
    activity = fluor_allcat_downsamp(use_trials,:,curr_roi,1);
    ...
    [kernel_flat,activity_predicted_reshape,expl_var,activity_predicted_reduced_reshape] = AP_regresskernel( ...
        cellfun(@(x) x(:,~isnan(activity_reshape)),regressors_reshape,'uni',false), ...
        activity_reshape(~isnan(activity_reshape)),sample_shifts,lambda,zs,cvfold);
...
%     activity_predicted(~isnan(activity')) = activity_predicted_reduced_reshape(:,:,1);
    ...
    fluor_kernel(:,curr_roi) = cellfun(@(x,t) reshape(x,[],length(t)), ...
        mat2cell(kernel_flat,cellfun(@(x) size(x,1),regressors_reshape).*cellfun(@length,sample_shifts),1), ...
        sample_shifts,'uni',false);
...
% Stim regressors
a = permute(cell2mat(permute(fluor_kernel(1,1:use_svs),[1,3,2])),[3,2,1]);
b = reshape(svdFrameReconstruct(U_master(:,:,1:use_svs),reshape(a,use_svs,[])),size(U_master,1),size(U_master,2),[],size(a,3));
AP_image_scroll(b,sample_shifts{1}/(sample_rate/downsample_factor));
set(gcf,'Name','Stim');
axis image
caxis([-prctile(abs(b(:)),100),prctile(abs(b(:)),100)]);
colormap(brewermap([],'*RdBu'))
AP_reference_outline('ccf_aligned','k');

% Move onset regressors
a = permute(cell2mat(permute(fluor_kernel(2,1:use_svs),[1,3,2])),[3,2,1]);
b = reshape(svdFrameReconstruct(U_master(:,:,1:use_svs),reshape(a,use_svs,[])),size(U_master,1),size(U_master,2),[],size(a,3));
c = b;
AP_image_scroll(c,sample_shifts{2}/(sample_rate/downsample_factor));
set(gcf,'Name','Movement onset');
axis image
caxis([-prctile(abs(c(:)),100),prctile(abs(c(:)),100)]);
colormap(brewermap([],'*RdBu'))
AP_reference_outline('ccf_aligned','k');

% Reward regressors
a = permute(cell2mat(permute(fluor_kernel(3,1:use_svs),[1,3,2])),[3,2,1]);
b = reshape(svdFrameReconstruct(U_master(:,:,1:use_svs),reshape(a,use_svs,[])),size(U_master,1),size(U_master,2),[],size(a,3));
c = b;
AP_image_scroll(c,sample_shifts{3}/(sample_rate/downsample_factor));
set(gcf,'Name','Reward');
axis image
caxis([-prctile(abs(c(:)),100),prctile(abs(c(:)),100)]);
colormap(brewermap([],'*RdBu'))
...
for curr_rxn = 1:length(rxn_time_bins)
    for curr_trial_type = 1:size(trial_types,2)
        ...
        curr_data = squeeze(nanmean(fluor_allcat_predicted(curr_trials,:,1:use_svs,plot_align),1))';
        curr_px = svdFrameReconstruct(U_master(:,:,1:use_svs),curr_data);
...
mua_kernel = cell(length(regressors),n_depths);
mua_expl_var = cell(length(regressors),n_depths);
for curr_roi = 1:n_depths
    
    activity = mua_allcat_downsamp(use_trials,:,curr_roi,1);
...
%     activity_predicted(~isnan(activity')) = activity_predicted_reduced_reshape(:,:,1);
    activity_predicted = activity_predicted';    
    mua_allcat_predicted(use_trials,:,curr_roi,1) = activity_predicted;
    ...
    AP_print_progress_fraction(curr_roi,n_depths);
...
for curr_rxn = 1:length(rxn_time_bins)
    for curr_trial_type = 1:size(trial_types,2)
        ...
        curr_data = mua_allcat_predicted(curr_trials,:,:,plot_align);
        mua_trial_types(:,:,curr_trial_type,curr_rxn) = squeeze(nanmean(curr_data,1))';
...
for i = 1:size(trial_types,2)
   ...
   set(gca,'ColorOrder',copper(4));
   plot(mua_trial_types(:,:,i)','linewidth',2);
end
...
kernel_trials = trial_contrast_allcat > 0 & ...
    trial_side_allcat == -trial_choice_allcat & move_t > 0 & move_t < 0.5;
...
for curr_depth = 1:n_depths
    ...
    [fluor_k,~,explained_var] = ...
        AP_regresskernel(reshape(permute( ...
        fluor_allcat_downsamp(curr_kernel_trials,:,:,:), ...
        [2,1,4,3]),[],n_rois)', ...
        reshape(permute(mua_allcat_downsamp(curr_kernel_trials,:,curr_depth,:), ...
        [2,1,4,3]),[],1)',kernel_frames,lambda,zs,cvfold);
    
    fluor_k_reshape(:,:,curr_depth) = reshape(fluor_k,n_rois,[]);
       
    % Create design matrix of all time-shifted regressors
    regressor_design = repmat(reshape(permute( ...
        fluor_allcat_downsamp(curr_nonan_trials,:,:,:), ...
        [2,1,4,3]),[],n_rois), ...
        [1,1,length(kernel_frames)]);
    ...
    mua_allcat_predicted(curr_nonan_trials,:,curr_depth,:) = ...
        permute(reshape(predicted_spikes',length(t_downsample_diff),sum(curr_nonan_trials),2),[2,1,4,3]);
...
colormap(brewermap([],'*RdBu'));
caxis([-prctile(abs(fluor_k_px_max(:)),100),prctile(abs(fluor_k_px_max(:)),100)]);
axis image off;
