...
peri_event_v = permute(reshape(interp1(th,(Vn-Vh_Un)',peri_event_t,'previous'), ...
    length(align_times),length(t),[]),[3,2,1]);

event_px_mean = AP_svdFrameReconstruct(Un, ...
    nanmean(peri_event_v - nanmean(peri_event_v(:,t < 0,:),2),3));
...
% Get average fluorescence values over time in SM cortex
...
avg_im_roi = cell2mat(cellfun(@(x) ...
    (reshape(x,[],2)'*use_mask(:))/sum(use_mask(:)),avg_im_aligned,'uni',false)');
figure; hold on
plot(avg_im_roi(1,:),'color',[0,0,0.8],'linewidth',2);
plot(avg_im_roi(2,:),'color',[0.7,0,0.7],'linewidth',2);
...
for curr_animal = 1:length(animals)
    ...
    for curr_experiment = 1:length(experiments)
        ...
        fVdf_deconv = AP_deconv_wf(fVdf);
        pxd = reshape(svdFrameReconstruct(Ud,fVdf_deconv),[],size(fVdf_deconv,2));
        
        skip_frames = framerate*1;
        peak_thresh = nanstd(reshape(pxd(:,skip_frames:end-skip_frames),[],1));
        
        curr_prominence_map = nan(size(Ud,1),size(Ud,2));
        for curr_px = 1:size(pxd,1)
            ...
            curr_prominence_map(curr_px) = nanstd(prominences./widths);
        end
        
        prominence_maps{curr_animal}{curr_experiment} = curr_prominence_map;
    ...
    figure('Name',animal);
    c = [0,prctile(cell2mat(cellfun(@(x) x(:), ...
        prominence_maps{curr_animal}','uni',false)),100)*0.8];
    h = tight_subplot(1,length(prominence_maps{curr_animal}));
    for curr_day = 1:length(prominence_maps{curr_animal})
       subplot(h(curr_day));
       imagesc(prominence_maps{curr_animal}{curr_day});
       axis image off
       caxis(c);
       colormap(hot);      
       curr_day_rel = datenum(experiments(curr_day).day) - ...
           datenum(experiments(1).day);
