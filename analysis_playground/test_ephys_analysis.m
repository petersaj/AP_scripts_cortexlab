...
psth_smooth_norm = (psth_smooth-nanmean(psth_smooth(:,bins < 0),2))./ ...
    nanmean(psth_smooth(:,bins < 0),2);

figure; AP_stackplot(psth_smooth_norm(:,20:end-20)',bins(20:end-20), ...
    2,[],'k',depth_group_centers);
...
depth_psth = nan(max(depth_group),diff(raster_window)/psth_bin_size,length(unique(stimIDs)));
...
gw = gausswin(smooth_size,3)';
smWin = gw./sum(gw);
psth_smooth = cell2mat(permute(arrayfun(@(x) conv2(depth_psth(:,:,x), ...
    smWin, 'same'),1:size(depth_psth,3),'uni',false),[1,3,2]));
...
% ~~~~ 1a) By subtracting the average responses around light
...
light_t_timeline = interp1(sync_ephys,sync_timeline,light_t,'linear','extrap');
...
% ~~~~ 1b) Like above but subtract rolling median - BETTER
...
warning('Blue resets at the start of experiments - this could be wrong')
...
for curr_channel = 1:size(lfp,1)
    ...
    blue_on_lfp = interp1(lfp_t_timeline,lfp(curr_channel,:),blue_on_pull_t);
    ...
    violet_on_lfp = interp1(lfp_t_timeline,lfp(curr_channel,:),violet_on_pull_t);
    ...
    blue_light_remove = interp1(reshape(blue_on_pull_t',[],1),reshape(blue_on_lfp_baselinesub_med',[],1),lfp_t_timeline);
    violet_light_remove = interp1(reshape(violet_on_pull_t',[],1),reshape(violet_on_lfp_baselinesub_med',[],1),lfp_t_timeline);
    ...
    % Store the artifact-removed trace
    lfp_lightfix(curr_channel,:) = lfp(curr_channel,:) - (blue_light_remove + violet_light_remove);
    
    AP_print_progress_fraction(curr_channel,size(lfp,1));
end

% ~~~~ 1c) Subtract rolling median (all channels together - large memory)
light_t_timeline = interp1(sync_ephys,sync_timeline,sync(3).timestamps,'linear','extrap');
light_on = light_t_timeline(sync(3).values == 1);
light_off = light_t_timeline(sync(3).values == 0);
...
% ~~~~ 2) By regression (as effective as above but less efficient)
