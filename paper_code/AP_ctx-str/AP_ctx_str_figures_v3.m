...
%% Fig 1e: Striatum multiunit end-of-striatum depth aligned
...
for curr_regressor = 1:n_regressors
    ...
    for curr_depth = 1:n_depths
%        AP_errorfill([],nanmean(task_str_k_animal_max{curr_regressor}(:,curr_depth,:),3), ...
%            AP_sem(task_str_k_animal_max{curr_regressor}(:,curr_depth,:),3),str_col(curr_depth,:),0.5);
...
%% Fig 3x: Striatum task v cortex regression
...
%% Fig 4a,b: Striatum vs Cortex-predicted (and all task event fix)
...
% timeavg_labels = {'Move 1','Move 2'};
% timeavg_t = {[-0.05,0.05],[0.05,0.15]};
% timeavg_align = repmat({move_align},length(timeavg_t),1);
% timeavg_trial_conditions = ...
%     repmat({[trial_choice_allcat == -1,trial_choice_allcat == 1]},length(timeavg_t),1);
% timeavg_condition_colors = ...
%     repmat({[0.6,0,0.6;0,0.6,0]},length(timeavg_t),1);

% timeavg_labels = {'Outcome 1'};
% timeavg_t = {[0.05,0.15]};
% timeavg_align = repmat({outcome_align},length(timeavg_t),1);
% timeavg_trial_conditions = ...
%     repmat({[trial_outcome_allcat == 1,trial_outcome_allcat == -1]},length(timeavg_t),1);
% timeavg_condition_colors = ...
%     repmat({[0,0,0.6;0,0,0]},length(timeavg_t),1);
...
% curr_partial = 2;
% 
% curr_act_allcat = mua_allcat - mua_taskpred_reduced_allcat(:,:,curr_partial);
% curr_act_pred_allcat = mua_ctxpred_allcat - mua_ctxpred_taskpred_reduced_allcat(:,:,curr_partial);
% 
% task_fix = ...
%     (mua_taskpred_allcat - mua_taskpred_reduced_allcat(:,:,:,curr_partial)) - ...
%     (mua_ctxpred_taskpred_allcat - mua_ctxpred_taskpred_reduced_allcat(:,:,:,curr_partial));
% curr_act_pred_fix_allcat = curr_act_pred_allcat + task_fix;
...
trial_contrastside_allcat_exp = mat2cell(trial_contrastside_allcat,trials_recording,1);
...
for curr_exp = 1:length(trials_recording)
    ...
    for curr_stim_idx = 1:length(curr_stim)
        curr_trials = trial_contrastside_allcat_exp{curr_exp} == curr_stim(curr_stim_idx);
...
%% Fig Sx: Striatum vs Cortex-predicted (and all partial task fixes)
% UPDATE THIS TO BE COPIED FROM ABOVE
...
% (set predicted activity fix by all events)
task_fix = mua_taskpred_allcat - mua_ctxpred_taskpred_allcat;
task_fix_reduced = (mua_taskpred_allcat - mua_taskpred_reduced_allcat) - ...
    (mua_ctxpred_taskpred_allcat - mua_ctxpred_taskpred_reduced_allcat);

curr_act_pred_fix_allcat = mua_ctxpred_allcat + task_fix;
curr_act_pred_partialfix_allcat = mua_ctxpred_allcat + task_fix_reduced;
...
for curr_area_idx = 1:length(plot_areas)   
    ...
    for curr_timeavg = 1:length(timeavg_labels)
        ...
        curr_act_predpartialfix = mat2cell(...
            permute(cell2mat(arrayfun(@(trial) circshift( ...
            curr_act_pred_partialfix_allcat(trial,:,plot_area,:), ...
            timeavg_align{curr_timeavg}(trial),2),transpose(1:size(curr_act_pred_partialfix_allcat,1)),'uni',false)),[1,2,4,3]), ...
            use_split,length(t),length(task_regressor_labels));
        ...
        curr_act_pred_avg = cellfun(@(x) squeeze(nanmean(x(:,curr_event_t,:),2)),curr_act_pred,'uni',false);
        curr_act_predfix_avg = cellfun(@(x) squeeze(nanmean(x(:,curr_event_t,:),2)),curr_act_predfix,'uni',false);
        curr_act_predpartialfix_avg = cellfun(@(x) squeeze(nanmean(x(:,curr_event_t,:),2)),curr_act_predpartialfix,'uni',false);
        ...
        act_predpartialfix_predbinmean = ...
            nan(n_act_bins,length(use_split),size(trial_conditions,2),length(task_regressor_labels));
        for curr_partial = 1:length(task_regressor_labels)
            act_predpartialfix_predbinmean(:,:,:,curr_partial) = ...
                cell2mat(arrayfun(@(condition) ...
                cell2mat(cellfun(@(act,bins,trial_cond,use_trials) ...
                accumarray(bins(use_trials & trial_cond(:,condition)), ...
                act(use_trials & trial_cond(:,condition),curr_partial), ...
                [n_act_bins,1],@nanmean,cast(NaN,class(act))), ...
                curr_act_predpartialfix_avg,pred_trial_bins,trial_conditions_exp,pred_use_trials,'uni',false)'), ...
                permute(1:size(trial_conditions,2),[1,3,2]),'uni',false));
        end
        ...
        % (measured vs binned predicted with full task fix)
        subplot(1+length(task_regressor_labels),length(timeavg_labels), ...
            sub2ind(fliplr([1+length(task_regressor_labels),length(timeavg_labels)]),curr_timeavg,1));
        hold on;
        set(gca,'ColorOrder',timeavg_condition_colors{curr_timeavg});
        ...
        cond_col = timeavg_condition_colors{curr_timeavg};
        for curr_cond = 1:size(trial_conditions,2)
            AP_errorfill( ...
                squeeze(nanmean(act_pred_predbinmean(:,:,curr_cond),2)), ...
                squeeze(nanmean(act_predfix_predbinmean(:,:,curr_cond),2)), ...
        ...
        title({act_title,'Task-fixed'});
        
        % (measured vs binned predicted with partial task fixes)
        for curr_partial = 1:length(task_regressor_labels)
            subplot(1+length(task_regressor_labels),length(timeavg_labels), ...
                sub2ind(fliplr([1+length(task_regressor_labels),length(timeavg_labels)]),curr_timeavg,1+curr_partial));
            hold on;
            set(gca,'ColorOrder',timeavg_condition_colors{curr_timeavg});
            ...
            for curr_cond = 1:size(trial_conditions,2)
                AP_errorfill( ...
                    squeeze(nanmean(act_pred_predbinmean(:,:,curr_cond),2)), ...
                    squeeze(nanmean(act_predpartialfix_predbinmean(:,:,curr_cond,curr_partial),2)), ...
                    squeeze(AP_sem(act_predpartialfix_predbinmean(:,:,curr_cond,curr_partial),2)),cond_col(curr_cond,:),0.5,true);
            end
            
            xlabel(['Predicted (' num2str(plot_area) ')']);
            ylabel(['Measured (' num2str(plot_area) ')'])
            ylim(xlim); axis square;
            title({act_title,[task_regressor_labels{curr_partial} '-fixed']});
        end
        linkaxes(get(measured_v_pred_fig,'Children'),'xy');
...
%% Fig SX: Striatum v Cortex (passive choiceworld)
...
%% Fig S6: Cortex-predicted striatum error (passive fullscreen stim: trained/naive)
