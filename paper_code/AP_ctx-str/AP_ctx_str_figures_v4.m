...
for protocol = protocols 
    ...
    expl_var_cat = cell2mat(horzcat(ctx_str_expl_var{:}));
    figure('Name',protocol);
    errorbar(nanmean(expl_var_cat,2),AP_sem(expl_var_cat,2),'k','linewidth',2);
    xlabel('Striatal depth');
    ylabel('Fraction explained variance');

    % Flip time to be spike-lag (now fluor-lag) and concatenate
    k_px_cat = cell2mat(permute(cellfun(@(x) flip(x,3), ...
        horzcat(ctx_str_kernel{:})','uni',false),[2,3,4,5,1]));
    k_px = nanmean(k_px_cat,5);
    ...
    k_px_norm = k_px./max(reshape(k_px,[],1,size(k_px,3),size(k_px,4)),[],1);    
    k_px_com = sum(k_px_norm.*permute(1:n_aligned_depths,[1,3,4,2]),4)./sum(k_px_norm,4);
    ...
    % Plot temporal profile within ROI
    ...
    k_kernelroi = arrayfun(@(x) ...
        reshape(reshape(kernel_roi.bw(:,:,x),[],1)'* ...
        reshape(k_px_cat(:,:,:,x,:),size(k_px,1)*size(k_px,2),[]), ...
        size(k_px_cat,3),size(k_px_cat,5))./sum(sum(kernel_roi.bw(:,:,x))), ...
        1:n_aligned_depths,'uni',false);
    
    figure('Name',protocol);
    for curr_depth = 1:n_aligned_depths
        curr_data = k_kernelroi{curr_depth}./max(abs(k_kernelroi{curr_depth}),[],1);
        
        subplot(n_aligned_depths,1,curr_depth);
        AP_errorfill(t,nanmean(curr_data,2), AP_sem(curr_data,2),'k');
        line([0,0],ylim,'color','k','linestyle','--');
        line(xlim,[0,0],'color','k','linestyle','--');
        xlabel('Fluorescence lag:spikes');
        ylabel(['Str ' num2str(curr_depth) ' ROI weight']);
    end
