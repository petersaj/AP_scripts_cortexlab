...
for curr_ctxdepth = 1:size(gcamp6s_kernel_mean,1)
    plot(gcamp6s_kernel_ctxdepth.regression_t, ...
        squeeze(gcamp6s_kernel_norm(curr_ctxdepth,:,:)), ...
        'color',depth_col(curr_ctxdepth,:));
    plot(gcamp6s_kernel_ctxdepth.regression_t, ...
    gcamp6s_kernel_mean(curr_ctxdepth,:), ...
        'color',depth_col(curr_ctxdepth,:),'linewidth',2);
end
...
errorbar(nanmean(cortex_fluor_corr_cat,3), ...
    repmat(mua_depth',1,size(cortex_fluor_corr_cat,2)), ...
...
% Plot explained variance task vs cortex R2 by experiment
mua_var_exp = log10(cell2mat(cellfun(@(x) var(reshape(x,[],n_depths)),mua_exp,'uni',false)));
