...
%     line(repmat(best_lambda,1,2),ylim,'color','k');
    ...
    %% LFP/fluorescence cross-correlation and coherence
    ...
    mua_fluor_spectra_corr{curr_recording} = spectra_corr_full([1,4,2]);
    
    figure('Name',animal);
    colormap(hot);
    c = [min(reshape(cell2mat(mua_fluor_spectra_corr{curr_recording}),[],1)), ...
        max(reshape(cell2mat(mua_fluor_spectra_corr{curr_recording}),[],1))];
    
    subplot(1,3,1);
    imagesc(spect_f,spect_f,mua_fluor_spectra_corr{curr_recording}{1});
    xlabel('MUA frequency');
    ylabel('MUA frequency');
    axis square; caxis(c);
    
    subplot(1,3,2);
    imagesc(spect_f,spect_f,mua_fluor_spectra_corr{curr_recording}{2});
    ...
    %% LFP/fluorescence spectral correlation
    ...
    spectra_corr_full = mat2cell(corrcoef([lfp_spect_power',fluor_spect_power']),...
        repmat(length(spect_f),1,2),repmat(length(spect_f),1,2));
    lfp_fluor_spectra_corr{curr_recording} = spectra_corr_full([1,4,2]);
    
    figure('Name',animal);
    colormap(hot);
    c = [min(reshape(cell2mat(lfp_fluor_spectra_corr{curr_recording}),[],1)), ...
        max(reshape(cell2mat(lfp_fluor_spectra_corr{curr_recording}),[],1))];
    
    subplot(1,3,1);
    imagesc(spect_f,spect_f,lfp_fluor_spectra_corr{curr_recording}{1});
    xlabel('LFP frequency');
    ylabel('LFP frequency');
    axis square; caxis(c);
    
    subplot(1,3,2);
    imagesc(spect_f,spect_f,lfp_fluor_spectra_corr{curr_recording}{2});
    xlabel('Fluor frequency');
    ylabel('Fluor frequency');
    axis square; caxis(c);
