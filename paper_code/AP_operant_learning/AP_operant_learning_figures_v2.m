...
fluor_move_nostim_roi = cellfun(@(x) ...
    ...
    vertcat(fluor_move_nostim_leftward_all{:}),'uni',false);

roi_hemiflip = circshift(1:n_rois,n_rois/2);
fluor_move_nostim_roi_hemiratio = cellfun(@(x) arrayfun(@(roi) ...
    x(roi_hemiflip(roi),:)'\x(roi,:)',1:n_rois), ...
    fluor_move_nostim_roi,'uni',false);

% DOING HERE: REPLICATE ABOVE FOR EACH TRIAL (AND THEN CHECK WHY
% INCONSISTENT ACROSS DAYS?)
trial_move_hemiratio = cell2mat(cellfun(@(x,tr) repmat(permute(x,[1,3,2]), ...
    tr,1,1),fluor_move_nostim_roi_hemiratio,num2cell(trials_recording), ...
    'uni',false));
...
%% ^^ Task - average activity (hemidiff stim/move align pre/post learn)
