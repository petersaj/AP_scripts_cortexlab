...
figure; h = tiledlayout(length(plot_areas),max(vertcat(celltype_exp{:})));
for curr_area = 1:length(plot_areas)
    for curr_celltype = 1:n_celltypes
        nexttile;
        pie([class_frac(curr_area,:,curr_celltype), ...
...
for curr_area = 1:length(plot_areas)
    for curr_celltype = 1:n_celltypes
        ...
        curr_cells = unit_area_idx_allcat == curr_area & ...
            celltype_allcat == curr_celltype;
        
        plot_cells = cat(1, ...
            find(curr_cells & stim_sig_allcat & ~move_sig_allcat), ...
            find(curr_cells & stim_sig_allcat & move_sig_allcat), ...
            find(curr_cells & ~stim_sig_allcat & move_sig_allcat), ...
            find(curr_cells & ~stim_sig_allcat & ~move_sig_allcat));
