...
stim_frac = cell2mat(permute(cellfun(@(area,celltype,stim_sig,move_sig) ...
    accumarray([area(area ~= 0),celltype(area ~= 0)+1], ...
...
class_frac = cat(3,nanmean(stim_frac,3),nanmean(stimmove_frac,3),nanmean(move_frac,3));
class_col = [0.8,0,0;0.8,0.5,0.5;0.5,0.5,0.5;1,1,1];

figure; h = tiledlayout(length(plot_areas),length(celltype_label));
for curr_area = 1:length(plot_areas)
    for curr_celltype = 1:length(celltype_label)
        nexttile;
        pie([class_frac(curr_area,:,curr_celltype),1-sum(class_frac(curr_area,:,curr_celltype))]);
