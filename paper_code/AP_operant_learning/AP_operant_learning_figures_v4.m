...
narrow_waveform_exp = cellfun(@(x) x <= waveform_duration_cutoff, ...
...
stim_frac = permute(cellfun(@(area,narrow_wave,stim_sig,move_sig) ...
    accumarray([area(area ~= 0),narrow_wave(area ~= 0)+1], ...
    stim_sig(area ~= 0) & ~move_sig(area ~= 0), ...
    [length(plot_areas),2],@mean), ...
    unit_area_idx,narrow_waveform_exp,stim_sig,move_sig,'uni',false,[3,2,1]));
