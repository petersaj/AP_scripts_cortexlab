...
% Get day relative to last task day
last_task_day{curr_animal} = task_last_day;
        trial_data_all.task_relative_day{curr_animal}(curr_day) = datenum(day);

% Store day relative to last task day
task_relative_day = cellfun(@(day,taskday) day' - taskday, ...
    recording_day,last_taskday,'uni',false)';
trial_postlearn_day = cell2mat(cellfun(@(x,day) cell2mat(cellfun(@(x,day) ...
    day*ones(size(x,1),1),x,num2cell(day),'uni',false)), ...
    wheel_all,postlearn_day_relative,'uni',false));
...
postlearn_day_unique = unique(cat(1,postlearn_day_relative{:}));
if ~all(postlearn_day_unique == cat(2,postlearn_day_relative{:}),'all')
...
for curr_day = 1:size(stim_px_dayavg_tmax,3)
        ...
        title(sprintf('Day %d',postlearn_day_unique(curr_day)));
end
linkaxes(allchild(h),'xy');
colorbar;
