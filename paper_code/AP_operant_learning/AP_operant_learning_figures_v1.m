...
% Index each trial for animal/day/daysplit and concat
n_daysplit = 4;
...
rxn_measured_frac = accumarray(cell2mat(cat(1,trial_split_idx{:})), ...
...
rxn_alt_frac = cell2mat(permute(arrayfun(@(x) ...
    accumarray(cell2mat(cat(1,trial_split_idx{:})), ...
...
rxn_alt_learn_prctile = prctile(rxn_alt_learn_mean,[5,95],2);
AP_errorfill(learned_day_grp(plot_learned), ...
    nanmean(rxn_alt_learn_mean(plot_learned,:),2), ...
    rxn_alt_learn_prctile(plot_learned,:),'r',[],false);
