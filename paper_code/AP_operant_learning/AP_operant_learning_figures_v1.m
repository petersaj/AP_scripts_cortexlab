...
% (discretize whisker movement within recording on used trials)
...
use_trials_recording = mat2cell(use_trials,trials_recording);
use_recordings = cellfun(@(whisker,use_trials) ...
    ~all(isnan(whisker)) & sum(use_trials) > min_trials,...
    whisker_recording,use_trials_recording);

n_whisker_grps = 4;
whisker_grp = cellfun(@(x) nan(size(x)),whisker_recording,'uni',false);
whisker_grp(use_recordings) = cellfun(@(whisker,use_trials) ...
    discretize(whisker, ...
    prctile(whisker(use_trials),linspace(0,100,n_whisker_grps+1))), ...
    whisker_recording(use_recordings), ...
    use_trials_recording(use_recordings),'uni',false);
...
curr_axes = allchild(h);
title(h,'Activity grouped by whisker movement');
for i = 1:length(plot_rois)+1
    linkaxes(curr_axes(i:length(plot_rois)+1:end),'xy');
end
