...
rxn_bins = [0:0.02:0.5];
warning('TEMP: logspace rxn bins')
rxn_bins = logspace(log10(0.05),log10(20),20);
...
% %%%%%%%%%%%% TESTING: plot a few days, not just pre/post
...
% 
% figure;
% h = tiledlayout(length(day_grps)-1,2,'TileIndexing','columnmajor');
% 
...
%         1:n_rxn_altsample,'uni',false),[1,3,2]));
% 
%     animal_rxn_measured_cathist = animal_rxn_measured_cathist - ...
%         nanmean(animal_rxn_alt_cathist,3);
%     animal_rxn_alt_cathist = animal_rxn_alt_cathist - ...
%         nanmean(animal_rxn_alt_cathist,3);
% 
%     nexttile; hold on;set(gca,'XScale','log');
%     xlabel('Reaction time');
%     ylabel('Frequency-null');
%     title(sprintf('Day %d-%d',day_grps(curr_day_grp),day_grps(curr_day_grp+1)-1));
% 
% %     AP_errorfill(rxn_bin_centers,nanmean(animal_rxn_measured_cathist,2), ...
% %         AP_sem(animal_rxn_measured_cathist,2),'k');
...
% Plot histogram by day and average post-learning histogram
figure;
...
xlabel('Reaction time');
ylabel('Day');
set(gca,'view',[90,-90],'YDir','normal');
...
AP_errorfill(rxn_bin_centers,nanmean(nanmean(animal_rxn_alt_cathist,2),3), ...
    animal_rxn_alt_cathist_ci,[0.5,0.5,0.5],[],false);
area(rxn_window,repmat(max(ylim),2,1),'linestyle','none', ...
    'FaceColor',[0.8,0.9,0.8]);
set(gca,'children',circshift(get(gca,'children'),-1),'view',[90,-90],'XTick','');
ylabel(sprintf('Probability (day %d+)',postlearning_day));
legend({'Fast reaction','Null','Measured'},'location','ne');
...
%%%%%% TRY HERE? PDF/CDF for all mice with null by day
...
% end
% 
% learned_day_unique = unique(learned_day_x)';
% [~,learned_day_id] = ismember(learned_day_x,learned_day_unique);
% 
% learned_day_idx = repmat(permute(learned_day_id,[1,3,2]),[1,length(rxn_bin_centers)]);
% [~,bin_idx,animal_idx] = ndgrid(1:size(animal_rxn,1),1:size(animal_rxn,2),1:size(animal_rxn,3));
% 
% rxn_hist_learned_day = accumarray([bin_idx(:),learned_day_idx(:)],animal_rxn(:),[],@nanmean,NaN);
% 
% figure; hold on;
% plot(rxn_bin_centers,rxn_hist_learned_day);
% plot(rxn_bin_centers,rxn_hist_learned_day(:,learned_day_unique == -2),'g','linewidth',2);
% plot(rxn_bin_centers,rxn_hist_learned_day(:,learned_day_unique == -1),'k','linewidth',2);
% plot(rxn_bin_centers,rxn_hist_learned_day(:,learned_day_unique == 0),'b','linewidth',2);
% plot(rxn_bin_centers,rxn_hist_learned_day(:,learned_day_unique == 1),'r','linewidth',2);
...
%%%%%%%%%%%%% TESTING: KS stat instead of defining window

x = cell(size(bhv));
xa = cell(size(bhv));

for curr_animal = 1:length(bhv)
    for curr_day = 1:length(rxn_measured{curr_animal})
        [~,~,x{curr_animal}(curr_day,1)] = ...
            kstest2(rxn_measured{curr_animal}{curr_day}, ...
            rxn_alt{curr_animal}{curr_day}(:,1));
        [~,~,xa{curr_animal}(curr_day,1)] = ...
            kstest2(rxn_alt{curr_animal}{curr_day}(:,1), ...
            rxn_alt{curr_animal}{curr_day}(:,2));
    end
end
figure;hold on;cellfun(@plot,x);cellfun(@plot,xa);

x2 = AP_padcatcell(x);
xa2 = AP_padcatcell(xa);

[r,ld] = grpstats(x2(:),learned_day_x(:),{@nanmean,'gname'});
[ra,ld] = grpstats(xa2(:),learned_day_x(:),{@nanmean,'gname'});
ld = cellfun(@str2num,ld);
figure; hold on
stairs(ld,r);
stairs(ld,ra);
xline(0,'--');
...
for curr_animal = 1:length(bhv)
    for curr_day = 1:length(rxn_measured{curr_animal})
        for curr_quartile = 1:4

            curr_n_trials = length(rxn_measured{curr_animal}{curr_day});
            curr_trials = ceil((1:curr_n_trials)/(curr_n_trials/4)) == curr_quartile;

            [~,~,x{curr_animal}(curr_day,curr_quartile)] = ...
                kstest2(rxn_measured{curr_animal}{curr_day}(curr_trials), ...
                rxn_alt{curr_animal}{curr_day}(curr_trials,1));
        end
    end
end

x2 = reshape(permute(padarray(AP_padcatcell(x),[0,1],nan,'post'),[2,1,3]),[],length(bhv));
m = grpstats(x2(:),learned_daysplit_x(:));
figure;plot(learned_day_grp_daysplit,m)
xlabel('Learned day');
ylabel('KS stat');
...
for curr_roi = plot_rois
  for curr_stim = stim_unique'
    nexttile;
    AP_errorfill(t, ...
        squeeze(nanmean(stim_roi_avg_stage(curr_roi,:,plot_stages,stim_unique == curr_stim,:),5)), ...
        squeeze(AP_sem(stim_roi_avg_stage(curr_roi,:,plot_stages,stim_unique == curr_stim,:),5)), ...
        stage_col(plot_stages,:));
    xlabel('Time from stim (s)');
...
stim_roi_hemidiff_avg_stage = cell2mat(permute(cellfun(@(x,ld) ...
    cat(3, ...
    nanmean(x(:,:,1:ld-1,:),3), ...
    nanmean(x(:,:,n_naive+1:ld-1,:),3), ...
    nanmean(x(:,:,ld:end,:),3)),stim_roi_hemidiff_avg,num2cell(learned_day), ...
    'uni',false),[2,3,4,5,1]));
...
for curr_roi = plot_rois
    for curr_stim = stim_unique'
        curr_data = stim_roi_hemidiff_avg_stage(curr_roi,:, ...
            plot_stages,stim_unique == curr_stim,:);

        nexttile;
        AP_errorfill(t,squeeze(nanmean(curr_data,5)), ...
            squeeze(AP_sem(curr_data,5)),stage_col(plot_stages,:));
        xlabel('Time from stim (s)');
...
% (link ROI axes)
ax = reshape(allchild(h),length(stim_unique),length(plot_rois))';
...
% stim_roi_tmax = cellfun(@(x) x./max(x,[],2),stim_roi_tmax,'uni',false);
...
p1 = cellfun(@(x) plot(x(plot_roi,1:end-1,3),x(plot_roi,2:end,3),'.k','MarkerSize',5),stim_roi_tmax);
p2 = cellfun(@(x,ld) plot(x(plot_roi,ld-1,3),x(plot_roi,ld,3),'o','MarkerSize',5), ...
...
p2 = cellfun(@(x,ld) plot(x(plot_roi,ld-1,3),x(plot_roi,ld,3),'o','MarkerSize',10), ...
    stim_roi_tmax,num2cell(learned_day+n_naive));
p2 = cellfun(@(x,ld) plot(x(plot_roi,ld-2,3),x(plot_roi,ld-1,3),'+','MarkerSize',10), ...
    stim_roi_tmax,num2cell(learned_day+n_naive));
p2 = cellfun(@(x,ld) plot(x(plot_roi,ld,3),x(plot_roi,ld+1,3),'h','MarkerSize',10), ...
    stim_roi_tmax,num2cell(learned_day+n_naive));
line(xlim,xlim,'color','k');
xlabel('\DeltaF/F_0 day X');
ylabel('\DeltaF/F_0 day X+1');
...
AP_image_scroll([fluor_postlearn_px_avg, ...
...
% Set trials to use
use_trials = trial_learned_day >= 0 & trial_outcome_allcat == 1;
...
for curr_align = 1:length(use_align)
    ...
    accum_idx = cat(4,v_idx,t_idx,animal_idx);   
    curr_v_align_avg = accumarray( ...
        reshape(accum_idx(use_trials,:,:),[],size(accum_idx,4)), ...
        reshape(curr_v_align(use_trials,:),[],1), ...
        [n_vs,length(t),length(animals)], ...
        @nanmean,NaN('single'));

    % (animal average the last training stage defined above)
    curr_v_align_avg_t = ...
        interp1(t,nanmean(curr_v_align_avg,3)', ...
...
for curr_align = 1:length(use_align)   
    for curr_plot_t = 1:length(plot_t{curr_align})
        ...
        nexttile;
        x_midpoint = size(U_master,2)/2;
        curr_align_px_hemidiff = align_px{curr_align}(:,:,curr_plot_t) - ...
            AP_reflect_widefield(align_px{curr_align}(:,:,curr_plot_t));
...
c_sum = (c1+c2)./max(c1(:)+c2(:));
c_diff = (c1-c2)./max(c1(:)-c2(:));

nexttile;imagesc(c_sum);caxis([-1,1]);colormap(gca,AP_colormap('BWR'));title('Sum');
nexttile;imagesc(c_diff);caxis([-1,1]);colormap(gca,AP_colormap('BWR'));title('Diff');

nexttile;
c = padarray(cat(3,c1,c2),[0,0,1],0,'post');
image(mat2gray(c))
title('RGB')
...
c_hue = max(0,g_hue-rescale(c_diff,r_hue,g_hue));
c_sat = mat2gray(c_sum);
c_val = 0.7*ones(size(c1));
...
% Plot trial activity
figure;
h = tiledlayout(2*max(trial_learned_stage)+2,length(plot_rois), ...
    'TileSpacing','compact','padding','compact');
for curr_stage = 1:max(trial_learned_stage)
    for curr_roi = plot_rois
        use_trials = find(trial_learned_stage == curr_stage);
...
% Average stim/move-aligned activity
roi_act_stim_learn_avg = accumarray( ...
    reshape(accum_stage_idx,[],size(accum_stage_idx,4)), ...
    curr_act(:), ...
    [max(learned_stage_idx(:)),length(t),n_rois,length(animals)], ...
    @nanmean,NaN('single'));

roi_act_move_learn_avg = accumarray( ...
    reshape(accum_stage_idx,[],size(accum_stage_idx,4)), ...
    curr_act_move(:), ...
    [max(learned_stage_idx(:)),length(t),n_rois,length(animals)], ...
    @nanmean,NaN('single'));

% Plot average activity (stim, then move)
stage_col = [0.5,0.5,0.5;0,0,0];
for curr_roi = plot_rois
    nexttile;
    AP_errorfill(t, ...
        permute(nanmean(roi_act_stim_learn_avg(:,:,curr_roi,:),4),[2,1,3]), ...
        permute(AP_sem(roi_act_stim_learn_avg(:,:,curr_roi,:),4),[2,1,3]), ...
        stage_col);
    axis tight; xlim(xlim+[-0.1,0.1]);
    xlabel('Time from stim (s)');
    ylabel('\DeltaF/F_0');
...
for curr_roi = plot_rois
    nexttile;
    AP_errorfill(t, ...
        permute(nanmean(roi_act_move_learn_avg(:,:,curr_roi,:),4),[2,1,3]), ...
        permute(AP_sem(roi_act_move_learn_avg(:,:,curr_roi,:),4),[2,1,3]), ...
        stage_col);
    axis tight; xlim(xlim+[-0.1,0.1]);
...
% stim_roi_act = repmat(fluor_roi_deconv(:,:,1:size(wf_roi,1)) - fluor_roi_deconv(:,:,size(wf_roi,1)+1:end),[1,1,2]);
% stim_regressor = strcmp(task_regressor_labels,'Stim');
