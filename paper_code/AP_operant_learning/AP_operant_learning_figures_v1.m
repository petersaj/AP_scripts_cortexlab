...
rxn_measured_mad = accumarray(trial_split_idx_cat(:,2:end), ...
    rxn_measured_cat, ...
    [max_days,length(bhv)],@(x) mad(x),NaN);
...
rxn_alt_mad_ci = squeeze(prctile(nanmean(rxn_alt_mad,2),[5,95],3));
AP_errorfill([],nanmean(rxn_alt_mad_ci(plot_days,:),2), ...
    rxn_alt_mad_ci(plot_days,:),[0.5,0.5,0.5],[],false);
errorbar(nanmean(rxn_measured_mad(plot_days,:),2), ...
    AP_sem(rxn_measured_mad(plot_days,:),2),'k','CapSize',0,'linewidth',2)
xlabel('Training day');
...
rxn_alt_mad_altdiff_ci = squeeze(prctile(nanmean(rxn_alt_mad_altdiff,2),[5,95],3));
AP_errorfill([],nanmean(rxn_alt_mad_altdiff_ci(plot_days,:),2), ...
    rxn_alt_mad_altdiff_ci(plot_days,:),[0.5,0.5,0.5],[],false);
errorbar(nanmean(rxn_measured_mad_altdiff(plot_days,:),2), ...
    AP_sem(rxn_measured_mad_altdiff(plot_days,:),2),'k','CapSize',0,'linewidth',2)
xlabel('Training day');
...
rxn_measured_mad = cellfun(@(x) mad(x),rxn_measured_cat);
rxn_alt_mad = cellfun(@(x) mad(x),rxn_alt_cat,'uni',false);
...
errorbar(nanmean(rxn_measured_mad_altdiff,2),AP_sem(rxn_measured_mad_altdiff,2),'k','linewidth',2);
xlim([0,n_conditions] + 0.5);
set(gca,'XTick',1:n_conditions,'XTickLabel',condition_labels);
ylabel('Reaction time MAD (s)');
...
%% ^^ Task - average activity (hemidiff learning change stim/move-align)
...
legend([ht,hpl,hpr],{'Task','Passive (L hemi/R stim)','Passive (R hemi/L stim)'},'location','nw');
