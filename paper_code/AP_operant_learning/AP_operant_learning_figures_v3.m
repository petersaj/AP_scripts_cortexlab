...
%% [FIG 3A]: ephys plot probe position
...
%% [FIG 3C]: ephys passive stim response
...
%% [FIG 3D] ephys move (no stim) response
...
%% [FIG 3E-F] ephys single cell classification and stim/move response
...
%% >> [FIG 4A1] task ROI timecourse by learned day
...
plot_rois = [6];
plot_learned_days = -3:2;

figure;
h = tiledlayout(length(plot_rois),length(plot_learned_days));
for curr_roi = plot_rois
...
%% >> [FIG 4C1] task hemidiff ROI average daysplit by learned day
...
%% ++ [FIG 4A2,C2] passive - ROI timecourse/stim window by learned day
...
% Plot ROI activity within stim window by learned day
use_t = t > 0 & t <= 0.2;
...
%% (run FIG 4C1-2) [FIG 4C combined]: task & passive daysplit
...
passive_errorbar = findall(fig_passive,'type','ErrorBar');
...
% (plot passive - just left and right stim)
plot_stim = [1,3];
stim_color = [0,0,0.8;0.5,0.5,0.5;0.8,0,0];
yyaxis right;
...
%% ++ [FIG S3 C-G]: passive whisker/pupil and ROI activity
