...
for curr_animal = 1:length(animals)
    for curr_day = find(use_days{curr_animal})'
                ...
                curr_alt_rxn = cell2mat(cellfun(@(x) ...
                    datasample(x,min(length(x),1)), ...
                    bhv(curr_animal).alt_stim_move_t{curr_day},'uni',false));
        
                [h,p(curr_animal,curr_day),ks2stat] = ...
                    kstest2(curr_rxn,curr_alt_rxn);        
    end
end
[~,learned_day] = max(p < 0.05,[],2);

%%% TESTING KS STATs (multisample)
% %%% TESTING KS STATS
max_days = max(cellfun(@sum,use_days));
p = nan(length(animals),max_days);
for curr_animal = 1:length(animals)
