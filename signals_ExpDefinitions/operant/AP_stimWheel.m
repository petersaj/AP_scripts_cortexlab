function AP_stimWheel(t, events, parameters, visStim, inputs, outputs, audio)
% AP_stimWheel(t, events, parameters, visStim, inputs, outputs, audio)
% 200901 - AP
%
% Modified from choiceworld: no audio, no stim movement, just stim comes up
% and then any wheel movement gives reward
%
% Task structure: 
% Start trial
...
% ** Any wheel movement brings stim to center
...
startingContrasts = [true,true,false,false,false];
% (which contrasts to repeat on miss)
repeatOnMiss = [true,true,false,false,false];
...
% Azimuth control
% 1) stim on side
% 2) at any response, stim fixed to center during reward
stimAzimuth = cond( ...
    events.newTrial.to(interactiveOn), startingAzimuth*trialData.trialSide, ...
    response.to(events.newTrial), 0);
...
% (in this protocol: always a hit)
trialData.hit = abs(stimDisplacement) > 0;
