classdef ball_bounce_ExpPanel < eui.ExpPanel
  ...
  methods
    function obj = ball_bounce_ExpPanel(parent, ref, params, logEntry)
...
%             time = datenum(updates(ui).timestamp);
    ...
    function expUpdate(obj, rig, evt)
      ...
      switch type
        case 'signals' %queue signal updates
          ...
          curr_ball_position = evt.Data(strmatch('events.ball_position',{evt.Data.name})).value;
          set(obj.visStimPlot.dot,'XData',curr_ball_position(1), ...
              'YData',curr_ball_position(2));
    ...
    %%%%% AP 2017-03-30: PLOTTING ATTEMPT %%%%%   
    
    obj.visStimPlot.axes = bui.Axes(obj.CustomPanel);
    
    % Make the visStimAxes be one pixel per degree
    %obj.visStimPlot.axes.ActivePositionProperty = 'position';
    obj.visStimPlot.axes.XLim = [-180 180];
    obj.visStimPlot.axes.YLim = [-90,90];
    
    obj.visStimPlot.dot = plot(0,0,'.k','MarkerSize',100);   
