classdef vanillaChoiceworldBiasLowContrastExpPanel < eui.ExpPanel
  ...
  methods
    function obj = vanillaChoiceworldBiasLowContrastExpPanel(parent, ref, params, logEntry)
    ...
    function expUpdate(obj, rig, evt)
      ...
      switch type
        case 'signals' %queue signal updates
          ...
          biasIdx = strmatch('events.bias',{evt.Data.name});
          if ~isempty(biasIdx)
              curr_leftward_bias = evt.Data(biasIdx).value/2+0.5;              
              set(obj.performancePlot.bias,'YData',[curr_leftward_bias,curr_leftward_bias]);
          end
...
%       obj.InfoGrid.ColumnSizes = [150, -1];
    ...
    obj.performancePlot.bias = line([-1,1],[0.5,0.5],'color','r');
