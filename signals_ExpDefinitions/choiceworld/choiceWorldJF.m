function choiceWorldJF(t, events, p, visStim, inputs, outputs, audio)
...
% Sounds
audioDevice = audio.Devices('Strix');%QQ
onsetToneFreq = 12000;%QQ
...
missNoiseDuration = 0.5; %QQ 2s in vanilla 
...
stim = vis.grating(t, 'square', 'gaussian');%QQ
stim.sigma = p.sigma;
...
p.responseDisplacement = 90; % (degrees)%QQ
...
% Timing
p.prestimQuiescentTime = 0.5; % (seconds) %QQ
...
% Audio
p.missNoiseAmplitude = 0.03;%QQ
p.onsetToneAmplitude = 0.3;%QQ
catch
end
end
...
if length(expRef) > 1
    ...
    for check_expt = length(expRef)-1:-1:1
        ...
        if exist(previousBlockFilename,'file')
            previousBlock = load(previousBlockFilename);
            if contains(previousBlock.block.expDef, 'vanilla'), continue, end
