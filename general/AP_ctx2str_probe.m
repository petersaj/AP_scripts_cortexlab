...
%% Directly compare regression and projection maps
...
depth_kernels = squeeze(r_px(:,:,find(kernel_frames == 0),:));
...
for curr_depth = 1:n_aligned_depths
    ...
    if ~isempty(curr_inj_idx)
        ...
        scatter(injection_coordinates_wf{curr_inj_idx}(:,1), ...
            ...
            'k','filled');
        scatter(injection_coordinates_wf{curr_inj_idx*2}(:,1), ...
            injection_coordinates_wf{curr_inj_idx*2}(:,2), ...
            [projection_strength_normalized{curr_inj_idx*2}*50 + 10], ...
            'k','filled');        
    end    
